"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorkflowRulesController = void 0;
const common_1 = require("@nestjs/common");
const workflow_rules_service_1 = require("./workflow-rules.service");
const create_workflow_rule_dto_1 = require("./dto/create-workflow-rule.dto");
const common_2 = require("../../../../common");
const jwt_auth_guard_1 = require("../auth/guards/jwt-auth.guard");
const tenant_1 = require("../../../../tenant");
const drizzle_orm_1 = require("drizzle-orm");
const drizzle_1 = require("../../../../drizzle");
let WorkflowRulesController = class WorkflowRulesController {
    workflowService;
    constructor(workflowService) {
        this.workflowService = workflowService;
    }
    async create(createWorkflowRuleDto) {
        return this.workflowService.insert(createWorkflowRuleDto);
    }
    async findAll(bucketId) {
        if (bucketId) {
            return this.workflowService.fetchActiveRulesForBucket(bucketId);
        }
        return this.workflowService.findMany();
    }
    async update(id, data) {
        return this.workflowService.update((0, drizzle_orm_1.eq)(drizzle_1.workflowRules.id, id), data);
    }
    async delete(id) {
        return this.workflowService.delete((0, drizzle_orm_1.eq)(drizzle_1.workflowRules.id, id));
    }
};
exports.WorkflowRulesController = WorkflowRulesController;
__decorate([
    (0, common_1.Post)(),
    (0, common_2.ApiResponseConfig)({
        message: 'Workflow rule created successfully',
        apiCode: 'WORKFLOW_RULE_CREATED',
    }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_workflow_rule_dto_1.CreateWorkflowRuleDto]),
    __metadata("design:returntype", Promise)
], WorkflowRulesController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    (0, common_2.ApiResponseConfig)({
        message: 'Workflow rules listed successfully',
        apiCode: 'WORKFLOW_RULES_LISTED',
    }),
    __param(0, (0, common_1.Query)('bucketId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], WorkflowRulesController.prototype, "findAll", null);
__decorate([
    (0, common_1.Put)(':id'),
    (0, common_2.ApiResponseConfig)({
        message: 'Workflow rule updated successfully',
        apiCode: 'WORKFLOW_RULE_UPDATED',
    }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], WorkflowRulesController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, common_2.ApiResponseConfig)({
        message: 'Workflow rule deleted successfully',
        apiCode: 'WORKFLOW_RULE_DELETED',
    }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], WorkflowRulesController.prototype, "delete", null);
exports.WorkflowRulesController = WorkflowRulesController = __decorate([
    (0, common_1.Controller)('workflow-rules'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, tenant_1.TenantGuard),
    __metadata("design:paramtypes", [workflow_rules_service_1.WorkflowRulesService])
], WorkflowRulesController);
//# sourceMappingURL=workflow-rules.controller.js.map